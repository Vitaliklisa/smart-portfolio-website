...
export default function AIChatBox({ open, onClose }: AIChatBoxProps) {
  ...
  return (
    <div
      className={cn(
        "x1:right-36 bottom-0 right-0 z-50 w-full max-w-[500px] p-1",
        open ? "fixed" : "hidden",
      )}
    >
      ...
      <div className="flex h-[600px] flex-col rounded border bg-background shadow-xl">
        <div className="mt-3 h-full overflow-y-auto px-3" ref={scrollRef}>
          ...
          {!error && messages.length === 0 && (
            <div className="mx-8 flex h-full flex-col items-center justify-center gap-3 text-center">
              ...
              <p>
                You can ask the Chatbot any queation about me and it will find
        ...
        <form onSubmit={handleSubmit} className="m-3 flex gap-1">
          <button
            type="button"
            className="flex w-10 flex-none items-center justify-center text-gray-500 hover:text-red-600 "
            title="Clear chat"
            onClick={() => setMessages([])}
          >
            <Trash size={24} />
